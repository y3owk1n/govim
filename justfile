...
fmt-check:
    ...
    find internal/bridge -name "*.h" -o -name "*.m" | xargs clang-format --dry-run -Werror || (echo "Some Objective-C files are not properly formatted. Run 'just fmt' to fix them." && exit 1)
...
fmt:
    ...
    find internal/bridge -name "*.h" -o -name "*.m" | xargs clang-format -i
