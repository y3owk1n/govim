...
fmt-check:
    ...
    @find internal/bridge \( -name "*.h" -o -name "*.m" \) -print0 | xargs -0 clang-format --dry-run -Werror --style=file --assume-filename=file.m || (echo "Some Objective-C files are not properly formatted. Run 'just fmt' to fix them." && exit 1)
...
fmt:
    ...
    @find internal/bridge \( -name "*.h" -o -name "*.m" \) -print0 | xargs -0 clang-format -i --style=file --assume-filename=file.m
